...
def sample_noise(batch_size, dim, seed=None):
    ...
    noises = 2 * torch.rand((batch_size, dim)) - 1
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = bce_loss(logits_fake, fake_labels) + bce_loss(logits_real, real_labels)
...
def generator_loss(logits_fake):
    ...
    # 这里让fake_labels为1是因为他不需要被1减掉，而取最小化损失
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        ...
        nn.Conv2d(in_channels = 1, out_channels = 32, kernel_size = 5, stride = 1), # (B, 32, 24, 24)
        nn.LeakyReLU(),
        nn.MaxPool2d(2, 2),     # (B, 32, 12, 12)
        nn.Conv2d(32, 64, 5, 1),# (B, 64, 8, 8)
        nn.LeakyReLU(),
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss, loader_train, show_every=250,
    ...
    for epoch in range(num_epochs):
        # loader_train是一个dataloader，返回的第一个参数是数据，第二个参数是label
