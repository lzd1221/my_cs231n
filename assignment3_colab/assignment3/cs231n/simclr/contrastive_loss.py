...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # 左边归一化
    left_norm = out_left / torch.linalg.norm(out_left, dim=1, keepdim=True) # (N, D)
    # 右边归一化
    right_norm = out_right / torch.linalg.norm(out_right, dim=1, keepdim=True)# (N, D)
    # sim_left * sim_right
    mul = torch.mm(left_norm, right_norm.T)     #(N, N)
    pos_pairs = torch.diag(mul).view(-1, 1)     # torch.diag 意味着输出mul的对角线，而后再用view将其变为(N,1)
    # 这里输出对角线是为了
