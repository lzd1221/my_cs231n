...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # 根据特征输入来得到图像向量    
        # 他这里特征已经提取好了，直接变成向量就行
        # 实际上在编码器部分就没有自注意力机制的使用。
        img_vec = self.visual_projection(features)  # (N, wordvec_dim)
        # 遮蔽T时刻之前的输入
        mask = torch.ones((T, T), dtype=bool)
        mask = torch.tril(mask)
        # 扩充维度
        img_vec = torch.unsqueeze(img_vec, 1)       # (N,wordvec_dim, 1)

        output = self.transformer(caption_embed, img_vec, mask) # 解码器
