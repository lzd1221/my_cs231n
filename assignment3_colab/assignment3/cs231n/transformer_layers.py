...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # 得分开shape来处理
        # 用torch.matmul
        # 用mask的话，要思考如何定义分数来避免value影响输出。
        # torhc的masked_fill可以覆盖。
        H = self.n_head
        # E / H
        H_hat = self.head_dim
        ...
        Q = Q.reshape((N, S, H, E // H)) # (N, S, H, E/H)
        K = K.reshape((N, T, H, E // H)) # (N, T, H, E/H)
        V = V.reshape((N, T, H, E // H)) # (N, T, H, E/H)

        Q = Q.permute(0, 2, 1, 3)       # (N, H, S, E/H)
        K = K.permute(0, 2, 1, 3)       # (N, H, T, E/H)
        V = V.permute(0, 2, 1, 3)       # (N, H, T, E/H)

        energy = torch.matmul(Q, K.permute(0, 1, 3, 2)) / math.sqrt(self.head_dim) # (N, H, T, T)
        if attn_mask is not None:
            energy.masked_fill_(attn_mask == False, -math.inf) #把需要填充的部分填充为负无穷大，最后softmax得到就是0
        attention = torch.softmax(energy, dim = 3) # 注意dim为3 要在T维度进行softmax，每一个时刻softmax一次。 #(N, H, T, T)
