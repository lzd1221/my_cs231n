...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    H = h0.shape[1]
    h = np.zeros((T, N, H))
    x_trans = np.swapaxes(x, 0, 1) #(N, T, D)->(T, N, D)
    for i in range(T):
        x_now = x_trans[i]
        if i == 0:
            h_now = h0
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dh0 += dh[:, i, :]
        dx_i, dh0, dWx_i, dWh_i, db_i = rnn_step_backward(dh0, cache[i])
        dx[i, :, :] = dx_i
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
...
def word_embedding_forward(x, W):
    ...
    out, cache = W[x], (x, W) #out.shape = (N, T, D)
...
def word_embedding_backward(dout, cache):
    ...
    # np.add.at() https://blog.csdn.net/qq120633269/article/details/110039585
    # 简而言之，数组(N, T)用数组(A, B)索引时，最后的大小为，(A, B, T)
    # np.add.at(a,b,c)的整个过程如下：
    # 创建一个形状为 a[b].shape 的一个全零数组，然后把 c 广播到其中
    # 让其和 a[b] 进行相加，然后再把相加的结果对应回到原来 a 的索引位置就可以了
    #     >>> np.zeros_like(a[b])    #先创建一个与a[b].shape的辅助array，元素全为0，并将c广播为与该辅助array相同shape,并进行相加
    # array([[[0, 0, 0, 0],	#这个会与[1, 2, 3, 4]相加,并将结果加到 a[1]
    #         [0, 0, 0, 0]],	#这个会与[5, 6, 7, 8]相加,并将结果加到 a[1]

    #        [[0, 0, 0, 0],	#这个会与[1, 2, 3, 4]相加,并将结果加到 a[0]
    #         [0, 0, 0, 0]],	#这个会与[5, 6, 7, 8]相加,并将结果加到 a[0]

    #        [[0, 0, 0, 0],	#这个会与[1, 2, 3, 4]相加,并将结果加到 a[1]
    #         [0, 0, 0, 0]]])	#这个会与[5, 6, 7, 8]相加,并将结果加到 a[1]
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, Wh, b, prev_c, prev_h, next_c)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, Wh, b, prev_c, prev_h, next_c = cache
    H = prev_h.shape[1]
    a = x.dot(Wx) + prev_h.dot(Wh) + b
    input_gate = a[:, :H]
    forget_gate = a[:, H: 2*H]
    ...
    da[:, H: 2 * H] = f*(1 - f) * df
    da[:, 2*H: 3*H] = o*(1 - o) * do
    da[:, 3*H: 4*H] = (1 - np.tanh(block_gate) ** 2) * dc
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        ...
        if i == 0:
            ...
            h_now = h[i-1, :, :]
            c_now = c[i-1, :, :]
        next_h, next_c, cache_now = lstm_step_forward(x_now, h_now, c_now, Wx, Wh, b)
        h[i, :, :] = next_h
        c[i, :, :] = next_c
        cache.append(cache_now)
    h = np.swapaxes(h, 0, 1)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        ...
        dx_i, dh0, dc0, dWx_i, dWh_i, db_i = lstm_step_backward(dh0, dc0, cache[i])
        dx[i, :, :] = dx_i
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
...
def temporal_affine_forward(x, w, b):
    ...
    """
    N, T, D = x.shape   # 输入的x
    M = b.shape[0]      # bia的shape， m表示这个x的需要计算损失的部分。
    out = x.reshape(N * T, D).dot(w).reshape(N, T, M) + b   #W*x + b
    cache = x, w, b, out
    return out, cache
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    probs = np.exp(x_flat - np.max(x_flat, axis=1, keepdims=True))              # 平移一下
    probs /= np.sum(probs, axis=1, keepdims=True)                               # 求出概率
    loss = -np.sum(mask_flat * np.log(probs[np.arange(N * T), y_flat])) / N     # 求最终loss。
    dx_flat = probs.copy()
    dx_flat[np.arange(N * T), y_flat] -= 1                                      # 正确类别要减1
    dx_flat /= N                                                                # 取平均
    dx_flat *= mask_flat[:, None]                                               # 把不需要计算的部分遮盖掉
