...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        #注意，这里的层数不只是两层，要用循环来初始化
        layer_dims = np.hstack((input_dim, hidden_dims, num_classes)) #很巧妙的方法
        #如何确定W和b的维度？
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == None:
                out, cache = affine_relu_forward(x, W, b) # cache[i]中缓存的是第i层神经网络的输入
            ##### batchnorm
            ...
            #### layer norm
                ...
                gamma = self.params['gamma' + str(i + 1)]
                beta = self.params['beta' + str(i + 1)]
                ln_params = self.ln_params[i]
                out, cache = affine_ln_relu_forward(x, W, b, gamma, beta, ln_params)
            ######dropout
            if self.use_dropout:
        ...
        # 也就是[0, num_layers - 1] 总共num_layers层
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            if self.use_dropout:
                dout = dropout_backward(dout, caches_dropout[i])
            if self.normalization == None:
