...
def affine_backward(dout, cache):
    ...
    db = np.sum(dout,axis = 0) #axis = 0 means the sum of rows
...
#     log_probs = score - np.log(score_sum) #避免除以0的问题
#     probs = np.exp(log_probs)
#     loss = -np.sum(np.log(probs[np.arange(num_train), y])) #直接算正确类的概率
...
# 我这段代码存在0/0的问题
# 下方的代码是抄的，不存在除以0的问题。
# 分析而言，我的问题主要是因为我没有直接除，而是先存起来再除，这样较小的sum可能会丢失精度，故出现了此问题
    num_train = x.shape[0]
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, gamma, sample_mean, sample_var, x_norm, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x, gamma, sample_mean, sample_var, x_norm, eps = cache
...
def layernorm_backward(dout, cache):
    ...
    dxnorm = dout * gamma
    dvar = np.sum(-0.5 * dxnorm * (x-x_mean) * ((x_var + eps)**(-1.5)), axis = 1)
    dmean = np.sum(-dxnorm*((x_var + eps)**(-0.5)), axis = 1)
    
    dx = dxnorm * ((x_var + eps)**(-0.5)) + dmean.reshape(-1, 1)/D + 2/D * dvar.reshape(-1, 1) * (x-x_mean) 
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out_W = int(1 + (W + 2 * pad - WW) / stride) # 注意转换为int
        ...
        for f in range(F):
            ...
            for hh in range(out_H):
                ...
                for ww in range(out_W):
                    out[n, f, hh, ww] = np.sum(w[f,:,:,:] * x_pad[n, :, hh*stride:hh*stride+HH, ww*stride: ww*stride+WW]) + b[f]
        ...
        for f in range(F):
            for hh in range(out_H):
                for ww in range(out_W):
                    ...
                    dw[f, :, :, :] += dout[n, f, hh, ww] * x_pad[n, :, hh*stride:hh*stride+HH, ww*stride: ww*stride+WW]
...
def max_pool_backward_naive(dout, cache):
    ...
    for hh in range(out_H):
        for ww in range(out_W):
            x_mask = x[:, :, hh*stride: hh*stride + pool_height, ww*stride: ww*stride + pool_width]
            max_mask = np.max(x_mask, axis = (2,3), keepdims = True)
            max_indices = (x_mask == max_mask)
            dx[:, :, hh*stride: hh*stride + pool_height, ww*stride: ww*stride + pool_width] += max_indices * (dout[:, :, hh:hh+1, ww:ww+1]) #这里注意必须是hh+1，不然维度不匹配，无法相乘。
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_norm = (x_group - x_mean) / np.sqrt(x_var + eps)
    x_norm = x_norm.reshape((N, C, H, W))
    
    out = gamma * x_norm + beta
    
    cache = (x, gamma, beta, x_mean, x_var, x_norm, G, eps, x_group)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_groupnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, gamma, beta, x_mean, x_var, x_norm, G, eps, x_group = cache
    ...
    dvar = np.sum(dx_groupnorm * -1.0 / 2 * (x_group - x_mean) / (x_var + eps) ** (3.0 / 2), axis=(2, 3, 4), keepdims=True)

    N_GROUP = C // G * H * W
    dmean1 = np.sum(dx_groupnorm * -1.0 / np.sqrt(x_var + eps), axis=(2, 3, 4), keepdims=True)
    dmean2_var = dvar * -2.0 / N_GROUP * np.sum(x_group - x_mean, axis=(2, 3, 4), keepdims=True)
    dmean = dmean1 + dmean2_var

    dx_group1 = dx_groupnorm * 1.0 / np.sqrt(x_var + eps)
    dx_group2_mean = dmean * 1.0 / N_GROUP
    dx_group3_var = dvar * 2.0 / N_GROUP * (x_group - x_mean)
